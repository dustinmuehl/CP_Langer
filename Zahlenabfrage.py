...
def Erzeuge_TrainiertesNetz(S, B, H, e): 
    #S Schrittweite |float
    #B BatchSize |float
    #H HiddenLayer Size |float
    #e Epochenanzahl fürs Training |float
    
    #Erzeugung eines eindeutigen Namen für die Textdatei
    name1 = "W1_aG"+ str(S) +str(B) +str(H) +str(e) + ".txt"
    name2 = "W2_aG"+ str(S) +str(B) +str(H) +str(e) + ".txt"
    
    n = NeuralNetwork(784, 30, 10)
    for i in range(e):
        n.feedforward(test_x.T[:, 4000])
        p = int(50000/B) 
        Gewichtsmatrix = n.train(train_x.T, train_y_dec.T, B, p, S)
    W1 = Gewichtsmatrix[0]
    W2 = Gewichtsmatrix[1]
    np.savetxt(name1, W1)
    np.savetxt(name2, W2)

def erkenne(pd): #pd pixeldatei
    S=3         #Schrittweite
    B=10        #Batch-Size
    e=2         #Epochen
    H=30        #Hidden-Layer Größe
    #Erzeugung eines eindeutigen Namen für die Textdatei
    name1 = "W1_aG"+ str(S) +str(B) +str(H) +str(e) + ".txt"
    name2 = "W2_aG"+ str(S) +str(B) +str(H) +str(e) + ".txt"
    #Überprüfung ob die Textdatei mit bereits berechneten Gewichten existiert
    if os.path.exists(name1) == False:  
        #Wenn Nein, wir die Funktion Erzeuge_TrainGewichte aufgerufen, um neue 
        #Gewichtsmatrizen zu erzeugen
        Erzeuge_TrainiertesNetz(S, B, H, e)
        n = NeuralNetwork(784, H, 10)
        n.W1 = np.loadtxt(name1)
        n.W2 = np.loadtxt(name2)
        Wert = n.feedforward(pd)           
        
    else:
        #Wenn Ja, werden die bereits berechneten Matrizen verwendet
        n = NeuralNetwork(784, H, 10)
        n.W1 = np.loadtxt(name1)
        n.W2 = np.loadtxt(name2)
        Wert = n.feedforward(pd)
        
    #Ausgabe der Wahrscheinlichkeiten für die erkannte Zahl
        
    for i in range(10):
        print("{:5.3f}".format(Wert[i])+"%   "+str(i))
...
def hypP(C, K, H, e):
    T = np.zeros((1,7))
    #C Schrittweite - Array | Laufindex j 
    #K Mini-Batch Size - Array | Laufindex b
    ...
    for h in range(len(H)):          #Für Hyperparameter HiddenLayer Größe
        ...
        for k in range(len(K)):          #für Hyperparamter Schrittweite und MB-Size
    
                for j in range(len(C)):      

                    start_proc = time.process_time()
                    for i in range(e):      #Epochen Durhlauf, Standard=20, für Studie=5
                        ...
                        n.train(train_x.T, train_y_dec.T, 10, 5000, 3)# K[k], 50000/K[k],C[j])
                        print("Epoche", i)
...
# e = 3         # Anzahl Epochen, zb e=4 damit Epoche 0,1,2,3 berechnet wird
# C = [2]       # Schrittweite
# K = [30, 20]  # Batch-Size
# H = [30]      # Hidden-Layer Size
# # txt_weight(H)
# # Ausgabe = hypP(C, K, H, e)
# # print(Ausgabe)
q = 9
erkenne(train_x[q])
print(train_y[q])
